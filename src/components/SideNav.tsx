    ...
    return (
        <nav className="sm:sticky -ml-1 sm:top-0 fixed inset-x-1 bottom-0 z-10 border-t sm:border-none dark:border-zinc-800  bg-white dark:bg-black sm:bg-inherit  px-1 sm:py-4 ">
            <ul className="flex gap-1 justify-between sm:justify-start sm:flex-col items-start ">
                <li className="">
                    <Link
                        className={`flex ${isCurrentPath('/') && "bg-gray-100 font-medium sm:bg-inherit"} gap-3 items-center  dark:hover:bg-zinc-800 hover:bg-gray-50 transition-colors duration-200  p-3 rounded-full`}
                        href="/"
                    >
                ...
                {!!user &&
                    (
                        <li className="">
                            <Link
                                className={`flex gap-2 ${isCurrentPath(`/profile/${user.id}`) && "bg-inherit font-medium dark:bg-zinc-800"} dark:hover:bg-zinc-800 items-center hover:bg-gray-50 transition-colors duration-200 p-3 rounded-full`}
